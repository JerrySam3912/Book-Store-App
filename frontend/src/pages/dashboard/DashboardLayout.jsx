...
const DashboardLayout = () => {
  ...
  const userRoleDisplay = userRole.toLowerCase(); // 'admin' or 'user'
  const userDisplayName = `${userName} (${userRoleDisplay})`; // Format: "Jerry Sấm (admin)"
  const userAvatar = currentUser?.avatar || currentUser?.avatar_url || null;
  ...
  return (
    ...
    <div className="flex-grow text-gray-800">
      <header className="flex items-center h-20 px-6 sm:px-10 bg-white">
        ...
        {/* Search bar removed - redundant with global search in Navbar */}
        <div className="flex flex-shrink-0 items-center ml-auto">
          <button className="inline-flex items-center p-2 hover:bg-gray-100 focus:bg-gray-100 rounded-lg">
            ...
            <span className="h-12 w-12 ml-2 sm:ml-3 mr-2 bg-purple-100 rounded-full overflow-hidden flex items-center justify-center">
              {userAvatar ? (
                <img src={userAvatar} alt="user profile photo" className="h-full w-full object-cover"/>
              ) : (
                <span className="text-purple-600 font-semibold text-lg">{userInitial}</span>
              )}
            </span>
