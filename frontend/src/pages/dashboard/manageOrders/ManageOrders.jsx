                  ...
                  <tr key={order._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-600 mt-1">
                        Method: {
                          order.paymentMethod === 'COD' ? 'Cash (COD)' : 
                          ...
                          Swal.fire({
                            ...
                            html: `
                              <div class="text-left" style="text-align: left;">
                                ...
                                <hr class="my-2">
                                <p><strong>Payment Method:</strong> <span style="color: #2563eb; font-weight: bold;">${
                                ...
                                ${order.voucher ? `
                                  <hr class="my-2">
                                  <p><strong>Voucher Applied:</strong></p>
                                  <p style="color: #16a34a; margin-left: 15px;">
                                    • Code: <strong>${order.voucher.code}</strong><br>
                                    • Name: ${order.voucher.name}<br>
                                    • Type: ${order.voucher.type}<br>
                                    • Discount: <strong>$${order.discountTotal?.toFixed(2) || '0.00'}</strong>
                                  </p>
                                ` : ''}
                                <hr class="my-2">
                                <p><strong>Items:</strong></p>
                                <ul class="list-disc list-inside" style="margin-left: 15px;">
                                  ${order.items?.map(item => `<li>${item.title} x${item.quantity} - $${(Number(item.totalPrice) || 0).toFixed(2)}</li>`).join('') || 'No items'}
                                </ul>
                                <hr class="my-2">
                                <p><strong>Subtotal:</strong> $${(Number(order.itemsTotal || order.totalPrice) || 0).toFixed(2)}</p>
                                ${order.discountTotal > 0 ? `<p><strong>Discount:</strong> <span style="color: #16a34a;">-$${order.discountTotal.toFixed(2)}</span></p>` : ''}
                                <p><strong>Shipping:</strong> $${(Number(order.shippingFee) || 0).toFixed(2)}</p>
                                <p><strong>Total:</strong> <span style="color: #dc2626; font-size: 1.2em; font-weight: bold;">$${(Number(order.totalPrice) || 0).toFixed(2)}</span></p>
                              </div>
                            `,
                            width: '600px',
                            icon: 'info',
